...
TOOL_PROMPT = """
answer the following by thinking carefully and diligently, step by step, delimited by <reasoning> & </reasoning>. if there are systemtools available, you may 'call' them by writing a tool operand delimited by <tool> & </tool>. submit the answer delimited by <answer> & </answer>.

here are the available tools:

{tool_descriptions}

tools can be called by writing a JSON-formatted operand between <tool> & </tool> delimiters. here's an example of a well formatted tool 'call':
<tool>
{{"name": "python", "args": {{"code": "import sympy\nx = sympy.symbols('x')\nprint(sympy.solve(x**2 - 4, x))"}}}}
</tool>
when tools are available, you will see a tool 'call's output delimited by <result> & </result>. You may call tools multiple times if needed.
